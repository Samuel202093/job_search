...
const Listing = () => {
  ...
  return (
    <div className='flex justify-between p-3 lg:p-5'>
        ...
        <IoMdArrowRoundBack className='previous absolute transition-all delay-150 ease-in-out text-3xl top-[1%] font-bold cursor-pointer hover:text-[#727171]' onClick={handlePrevPage}/>
        <span className='tooltip hidden absolute top-[1%] lg:top-[1%] left-[12%] md:left-[7%] lg:left-[4%] text-sm border-[1px] border-[black] px-2 py-1 transition-all delay-150 ease-in'>back to previous page</span>
      ...
      <div className='flex justify-centery min-h-[25vh] border-2y border-[red]y w-[100%] lg:w-[55%] p-2 lg:p-3 mt-4 lg:mt-0'>
        <div className='flex flex-col w-[100%] border-2y border-[black]y px-1 md:px-5 py-3 gap-3'>
            ...
            <span className='font-semibold'>{data.remote===false? "onsite":"remote"}</span>
            
            <p dangerouslySetInnerHTML={{__html: data.description}} className='description'/>
            
            {/* for mobile view */}
        <span className='lg:hidden bg-[#00acee] text-[#E4F2E7] flex self-end px-3 py-1 font-semibold rounded cursor-pointer' onClick={handleMobile}>Apply</span>
        </div>
      </div>
